...
class OrderCheckoutFlowTests(TestCase):
    def setUp(self):
        self.braga_location = Location.objects.create(name='Loja Braga', is_active=True, order=1)
        self.guimaraes_location = Location.objects.create(name='Loja Guimarães', is_active=True, order=2)
    ...
    @patch('apps.orders.views.stripe_service.create_checkout_session')
    def test_guest_order_pages_require_access_token(self, create_checkout_session):
        create_checkout_session.return_value = {
            'session_id': 'cs_test_100',
            'payment_intent_id': 'pi_test_100',
            'checkout_url': 'https://checkout.stripe.com/pay/cs_test_100',
        ...
        self.assertEqual(isolated_response.status_code, 404)
    ...
    def test_guest_query_string_token_access_is_rejected_and_does_not_grant_session_access(self):
        order = Order.objects.create(
            name='Marco',
            email='marco@example.com',
            fulfillment_method=Order.FulfillmentMethod.PICKUP,
        ...
        first_response = isolated_client.get(
            reverse('orders:payment_status', kwargs={'order_id': order.pk}) + f'?token={order.access_token}&session_id=cs_exchange_100',
            HTTP_HOST='loja.lvh.me',
        )

        self.assertEqual(first_response.status_code, 404)

        second_response = isolated_client.get(
            reverse('orders:payment_status', kwargs={'order_id': order.pk}),
            HTTP_HOST='loja.lvh.me',
        )
