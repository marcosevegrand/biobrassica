  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
    @forelse($locations as $location)
      ...
      <div class="bg-stone/15 rounded-lg p-8">
        ...
        @if($location->map_embed_url)<div class="mt-6 overflow-hidden rounded-sm border border-stone/20"><iframe src="{{ $location->map_embed_url }}" width="100%" height="250" class="border-0" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="{{ $location->name }}"></iframe></div>@endif
      </div>
    @empty
